...
final class KeychainManager {
    ...
    func saveInstance(_ instance: CeilInstance) {
    ...
    func getAllInstances() -> [CeilInstance] {
        guard let data = KeychainHelper.load(key: instancesKey),
              let instances = try? JSONDecoder().decode([CeilInstance].self, from: data) else {
            return []
        }
        return instances
    }
    ...
    private func saveInstances(_ instances: [CeilInstance]) {
    ...
    func getCurrentInstance() -> CeilInstance? {
        guard let data = KeychainHelper.load(key: currentInstanceKey),
              let instance = try? JSONDecoder().decode(CeilInstance.self, from: data) else {
            return nil
        }
        return instance
    }
